...
const SEED_TOKEN = 'nwt-update-april-2026';
...
export async function POST(request: Request) {
  try {
    ...
    for (const product of products) {
      ...
      results.push({ sku: result.sku, name: result.name, stock: result.stock, id: result.id });
    }

    return NextResponse.json({
      message: `${results.length} products upserted successfully`,
      products: results,
    });
  } catch (error) {
    console.error('Seed error:', error);
    return NextResponse.json({ error: 'Error running seed' }, { status: 500 });
  }
}
